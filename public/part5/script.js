...
// Helper function to clear the output area when using multichat mode
function clearOutputForMultichat(useMultichat) {
    // Get the output element
    const output = document.getElementById('output');
    // In multichat mode, don't display single responses in the output area
    // since the chat history is displayed instead
    if (useMultichat) {
        output.textContent = '';
    }
}
...
async function sendToGemini() {
    ...
    try {
        ...
        // Store assistant response and update display based on multichat mode
        if (useMultichat) {
            // In multichat mode: add response to history, render all chat messages, clear single-response output
            chatHistory.push({ role: 'assistant', content: assistantReply });
            renderChatHistory();
            output.textContent = '';
        } else {
            // In single-turn mode: just display the response directly
            output.textContent = assistantReply;
        }
    } catch (error) {
